...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <div className="w-full lg:w-72 bg-slate-950 h-full flex flex-col text-slate-400 border-r border-white/5">
      <div className="p-8">
        <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6">Operations</h2>
        <nav className="space-y-2">
          <button
            onClick={() => setView(AppView.DASHBOARD)}
            className={`w-full flex items-center px-4 py-3 text-sm font-bold rounded-xl transition-all ${
              currentView === AppView.DASHBOARD ? 'bg-white/5 text-white border border-white/10 shadow-lg' : 'hover:bg-white/5 hover:text-slate-200'
            }`}
          >
              ...
              {auditSections.map((section) => (
                <button
                  ...
                  className={`w-full flex items-center px-3 py-2 text-xs font-bold rounded-lg transition-all ${
                    activeTab === section.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-200 hover:bg-white/5'
                  }`}
                >
      ...
      <div className="flex-1 overflow-y-auto px-8 py-4">
        <div className="flex items-center justify-between mb-6">
          ...
          {onPrintAll && audits.length > 0 && (
            <button onClick={onPrintAll} className="text-[10px] text-rose-500 hover:text-rose-400 font-black uppercase tracking-widest transition-colors">
              Export
            </button>
          )}
        </div>
      ...
      <div className="p-8 border-t border-white/5 bg-slate-950/80 backdrop-blur-xl">
        <div className="flex items-center space-x-4">
          <div className="relative">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center text-xs font-black text-white shadow-lg shadow-indigo-500/20">
              MA
            </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.2em] mt-1 flex items-center">
              <span className="h-1 w-1 bg-emerald-500 rounded-full mr-1.5"></span>
              Operational
            </p>
          </div>
        </div>
